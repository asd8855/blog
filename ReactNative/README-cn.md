...
![CodePush CLI](img/codepush1.png)
...
4. [发布](#发布更新)更新
5. 检查[调试日志]()以确保一切正常工作
6. 活的长而成功！[详细资料](https://en.wikipedia.org/wiki/Vulcan_salute)

## 创建账号

在你发布应用更新之前，你需要创建一个CodePush帐号。如果你已经安装了Cli，你就可以简单的使用如下命令来注册：
...
*注意：注册成功后，你就已经自动登录了。所以除非你自己登出了，否则你不需要在此机器上再次登录。*
...
注意：为了可以关联多个账号，你在另一个身份认证供应商那边用的Email地址必须与你现存的帐号一致。
...
## 代理支持
默认情况下，```login```命令将自动查找通过HTTPS_PROXY 或者 HTTP_PROXY 环境变量系统范围代理，并使用它连接CodePush服务。如果您想禁用此行为，并让CLI建立一个到CodePush的直接连接，只需在登录时指定——noProxy参数:  

```
code-push login --noProxy
```
我希望您明确指定CodePush CLI应该使用的代理服务器，而不依赖于系统范围的设置，您可以在登录时传递——代理参数:

```
code-push login --proxy https://foo.com:3454
```

一旦您登录，任何推断和/或指定的代理设置都会随着您的用户会话一起持久化。这允许您继续使用CodePush CLI，而无需重新认证或重新指定首选代理。如果您希望在任何时候开始或停止使用代理，只需注销，然后使用新需要的设置重新登录。  
另外，如果您想要查看当前登录设置使用了哪些代理设置(如果有的话)，只需运行```code-push whoami```命令，它将显示您是否在使用，或者显式地绕过代理。
...
如果你将和其它开发者在一起共同开发同一个CodePush应用，你可以把他们添加为合作者，使用如下命令：
...
### 所有权转移
##### 2.0.0.0+ 版本
2.0.0.0版本的更新删除了app transfer命令。您仍然可以通过组织来管理应用程序的所有权转移。这需要您访问移动中心并执行一些步骤。   
1. 去[https://mobile.azure.com](https://mobile.azure.com),创建一个新的组织  
2. 邀请你希望将所有权转移这个组织的成员。一旦他们接受邀请将改变他们的权限为“Admin”  
3. 导航到您的应用程序，并单击“管理应用程序”按钮(当在应用程序的“开始”页面时)。点击传输按钮将应用程序转移到org。请注意，目前这个操作无法逆转，尽管将来会发生变化。  
4. 一旦你的受邀者接受了，选择你创建的组织并将你自己从其中移除。
...
*注意：根据语义版本规范，版本范围仅对非预发布版本生效：(https://github.com/npm/node-semver#prerelease-tags) 。当你想要发布更新到一个预发布的版本上时，则需要明确指定你想要提升的版本号（比如`1.2.3-beta`）。*
...
{
  "appVersion" : "1.0.1",
  "deploymentKey" : "gznakSDQMAWZMGj2Cdv875OuRQp-16019a3c-6414-4755-9fda-175132ca846e",
  "description" : "第三个更新包",
  "downloadUrl" : "https://codepushupdates.azureedge.net/storagev2/BnYDVLeQ7NlblEo8E0k0s5vEJ4g216019a3c-6414-4755-9fda-175132ca846e",
  "failedInstall" : false,
  "isMandatory" : true,
  "isPending" : true,
  "label" : "v3",
  "packageHash" : "9ef5447811f10e5f3381c2a000ee0d171fa4203a99db21dedfe0c3a3370f8c0a",
  "packageSize" : 269668
}
```
